...
async function main() {
  ...
  const ffaBtn  = document.getElementById('start-ffa');
  const teamBtn = document.getElementById('start-team');
  const mineBtn = document.getElementById('start-mining');
  if (ffaBtn)  ffaBtn.addEventListener('click',  () => begin('ffa'));
  if (teamBtn) teamBtn.addEventListener('click', () => begin('team'));
  if (mineBtn) mineBtn.addEventListener('click', () => begin('mining'));
}

...
> `.sh` files**. Typing `./nexus.sh dev` there makes Windows try to *open* the
...
> ```
>
> Both do exactly the same thing — `nexus.ps1` just hands the command to Git
...
- **Shortcut** — the `./nexus.sh` wrapper, which handles the Windows quirks for you
...
`./nexus.sh` detects Windows and applies both automatically, which is the main
reason to use it.
...
```
**Shortcut:** `./nexus.sh logs` · `./nexus.sh logs client`
...
```
**Shortcut:** `./nexus.sh test:protocol`
...
```
**Shortcut:** `./nexus.sh loadtest` (or `./nexus.sh loadtest 50`)
...
```
**Shortcut:** `./nexus.sh health` · `./nexus.sh metrics`

Just the game's own numbers:
```bash
curl -s http://localhost:2567/metrics | grep "^nexus_"
```

The one that matters most is `nexus_tick_duration_ms` — alert if its 99th
...
**Normally:** `npm install some-package --workspace @nexus/server`

**You type:**
```bash
MSYS_NO_PATHCONV=1 docker run --rm -v "$(pwd -W)":/app -w /app node:20-alpine \
  npm install some-package --workspace @nexus/server
```
...
**Shortcut for the rebuild:** `./nexus.sh reset`
...
```
**Shortcut:** `./nexus.sh prod:logs`

### Build the production image without deploying

```bash
MSYS_NO_PATHCONV=1 docker build -f infra/server.prod.Dockerfile -t nexus-server:test .
```
...
| Symptom | Command | Why |
|---|---|---|
| `Cannot find package 'x'` after adding a dependency | `./nexus.sh reset` | Old `node_modules` volume shadowing the new image |
| Server edits seem ignored | `./nexus.sh restart` | Watcher misses Windows bind-mount changes |
...
| Weird build errors, `parent snapshot does not exist` | `./nexus.sh clean` | Docker's build cache is corrupt |
| `error during connect: ... dockerDesktopLinuxEngine` | Start Docker Desktop | The engine isn't running |
| Nothing works, no idea why | `./nexus.sh clean` then `./nexus.sh dev` | Full rebuild from scratch |
...
| I want to… | Command |
|---|---|
| Play locally | `./nexus.sh dev` → http://localhost:5173 |
| Play with friends | `./nexus.sh playtest` |
| Stop everything | `./nexus.sh stop` |
| Run all tests | `./nexus.sh test` |
| See the logs | `./nexus.sh logs` |
| Restart after a server edit | `./nexus.sh restart` |
| Check bandwidth cost | `./nexus.sh bandwidth` |
| Check server capacity | `./nexus.sh loadtest` |
| Review a playtest | `./nexus.sh analyse` |
| Fix a broken build | `./nexus.sh reset` |
| Deploy for real | `./nexus.sh deploy` |
| See all of these | `./nexus.sh help` |

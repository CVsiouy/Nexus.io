...
export class AISystem {
  ...
  _threatNearBase(state, player) {
    return state.grid.nearest(
      player.base.position.x, player.base.position.y, 220,
      (e) => e.hp > 0 && state.areEnemies(player.id, e.ownerId),
    );
  }
  ...
  _countAttackers(state, ownerId, pos) {
    return state.grid.count(
      pos.x, pos.y, 260,
      (e) => e.hp > 0 && state.areEnemies(ownerId, e.ownerId),
    );
  }
  ...
  _teammateUnderAttack(state, player) {
    ...
    for (const [, ally] of state.players) {
      ...
      const pressed = state.grid.any(
        ally.base.position.x, ally.base.position.y, 220,
        (e) => e.hp > 0 && state.areEnemies(ally.id, e.ownerId),
      );
      if (!pressed) continue;

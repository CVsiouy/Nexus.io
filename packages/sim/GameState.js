import { POP_BASE, POP_PER_LEVEL, WORLD_SIZE } from './constants.js';
...
export class GameState {
  constructor(playerId) {
    ...
    /**
     * Entity IDs for THIS match only. Every `new Soldier(...)` etc. takes its
     * id from here via `state.newId()`, and every delete must hand it back via
     * `state.freeId()`. See IdAllocator.js for why this can't be a global.
     */
    this.ids        = new IdAllocator();

    /**
     * Spatial index over living soldiers, rebuilt twice per tick (once before
     * the systems run, once after movement). Every "who is near me?" question
     * goes through this instead of scanning all soldiers — see spatial/HashGrid.js
     * for why that matters so much.
     */
    this.grid       = new HashGrid(WORLD_SIZE);
  ...
  /** Take a fresh entity ID. */
  newId() { return this.ids.alloc(); }

  /** Return an entity ID after deleting the entity. Always pair with a delete. */
  freeId(id) { this.ids.free(id); }

  /** Resolve a target ID to an entity (soldier, base, boss, eatable, wildling) */
  resolve(id) {
    // NOTE: `id == null` rather than `!id` — IDs are numbers now, and while the
    // allocator never issues 0, a `!id` test would silently swallow it if it ever did.
